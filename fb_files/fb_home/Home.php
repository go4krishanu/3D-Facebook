...
<?php
	if(isset($_POST['delete_warning']))
	{
		...
		mysqli_query("delete from user_warning where user_id=$user_warning_id;");
	}
	...
	if(isset($_POST['txt']))
	{
		...
		mysqli_query("insert into user_post(user_id,post_txt,post_time,priority) values('$userid','$txt','$post_time','$priority');");
	}
	...
	if(isset($_POST['file']) && ($_POST['file']=='post'))
	{
    	...
    	mysqli_query("insert into user_post(user_id,post_txt,post_pic,post_time,priority) values('$userid','$txt','$img_name','$post_time','$priority');");
	}
	if(isset($_POST['delete_post']))
	{
		$post_id=intval($_POST['post_id']);
		mysqli_query("delete from user_post where post_id=$post_id;");
	}
	if(isset($_POST['Like']))
	{
...
<body id="body">
<?php
	$que_warning=mysqli_query("select * from user_warning where user_id=$userid");
...
<?php
	$que_notice=mysqli_query("select * from users_notice where user_id=$userid");
...
<option value="Private"> Only me </option> 
	...
	<table cellspacing="0">
<?php
	$que_post=mysqli_query("select * from user_post where priority='Public' order by post_id desc");
	while($post_data=mysqli_fetch_array($que_post))
	{
		...
		$que_user_info=mysqli_query("select * from users where user_id=$post_user_id");
		$que_user_pic=mysqli_query("select * from user_profile_pic where user_id=$post_user_id");
...
<?php
	...
	<tr style="color:#6D84C4;">
		<td >   </td>
		<?php
		 	$que_status=mysqli_query("select * from user_post_status where post_id=$postid and user_id=$userid;");
			$que_like=mysqli_query("select * from user_post_status where post_id=$postid");
		...
		<input type="submit" value="Like" name="Like" style="border:#FFFFFF; background:#FFFFFF; font-size:15px; color:#6D84C4;" onMouseOver="like_underLine(<?php echo $postid; ?>)" onMouseOut="like_NounderLine(<?php echo $postid; ?>)" id="like<?php echo $postid; ?>"></form></td>
		 ...
		 <?php
		 	...
		 	$que_comment=mysqli_query("select * from user_post_comment where post_id =$postid order by comment_id");
